...
public class Proxy extends UnicastRemoteObject implements SipListener, RemoteServiceInterface, Runnable
{
    ...
    private void processIncomingRequest(final RequestEvent requestEvent) throws InvalidArgumentException, ParseException, SipException
    {
		...
		try{
		    	...
		    	SIPRequest sr = (SIPRequest) request;
		    	String sdpContent;
		    if(sr.getRawContent() != null) 
			{
				String callid = ((CallIdHeader) request.getHeader(CallIdHeader.NAME)).getCallId();
				log.trace("CALLID" + callid);
		        sdpContent = new String(sr.getRawContent());
		        SdpFactory sdpf = SdpFactory.getInstance();
		        SessionDescription requestSDP;
		        if( (requestSDP = sdpf.createSessionDescription(sdpContent)) != null )
				{
		            log.trace("######################Notre affichage s'affiche######################");
		            log.trace("##########################" + requestSDP.toString() + "#################");
					msgBB += "callid" + callid + "callid";
					msgBB += requestSDP.toString();
					log.trace("################Début msgBB INVITE##############");
					log.trace(msgBB);
					log.trace("################Fin msgBB INVITE##############");
					msgBB += "###";
		    	}
			}
			} catch (Exception e) {
		...
		{
			try{
			String callidbye = ((CallIdHeader) request.getHeader(CallIdHeader.NAME)).getCallId();
			log.trace("CALLID" + callidbye);
			sendBYE("callid" + callidbye + "callid" +  "BYE");
			} catch(Exception e) {
    ...
    public boolean validateRequest(Request request, SipProvider sipProvider, ServerTransaction serverTransaction, LocationServiceInterface locationService) throws InvalidArgumentException, SipException, ParseException
    {
         ...
         * recipient and respond per Section 11.  Otherwise, the element MUST192.168.1.3
     ...
     * @param request Request192.168.1.3
     * @param dsam
	...
	private void sendInfoBB(String message) throws Exception{
		try{
			Socket socketClient = new Socket("192.168.1.20",9000);
			PrintWriter out = new PrintWriter(socketClient.getOutputStream(),true);
			out.println(message);
			
			ServerSocket socketServeur = new ServerSocket(9000);
			System.out.println("Lancement du serveur");

			socketClient = socketServeur.accept();
			String msg_receive = "";
				
			System.out.println("Connexion avec 192.168.1.20");
				
			BufferedReader in = new BufferedReader(new InputStreamReader(socketClient.getInputStream()));
			PrintStream output = new PrintStream(socketClient.getOutputStream());
			msg_receive = in.readLine();
			output.println(msg_receive);
	...
	private void sendBYE(String message) throws Exception{
		try{
			Socket socket = new Socket("192.168.1.20",9000);
		...
		try{
		    ...
		    if(sr.getRawContent() != null) 
			{
		        ...
		        if( (requestSDP = sdpf.createSessionDescription(sdpContent)) != null )
				{
		            ...
		            log.trace("##########################" + requestSDP.toString() + "#################");
					msgBB += requestSDP.toString();
					log.trace("################Début msgBB Response##############");
					log.trace(msgBB);
					log.trace("################Fin msgBB Response##############");
					sendInfoBB(msgBB);
		    	}
			}
			} catch (Exception e) {
