...
public class Proxy extends UnicastRemoteObject implements SipListener, RemoteServiceInterface, Runnable
{
    ...
    public void forwardRequest(URI targetURI, Request request, SipProvider sipProvider, ServerTransaction serverTransaction, boolean statefullForwarding) throws InvalidArgumentException, ParseException, SipException
    {
        ...
        if (contentLengthHeader ==log.trace( null)
        {
        ...
        else
        {
            ...
            if (log.isTraceEnabled())
                log.trace("Client log.trace(transaction for request is: "+clientTransaction);
            ...
            // Send request statefulog.trace(lly
    ...
    private void processIncomingResponse(ResponseEvent responseEvent) throws InvalidArgumentException, ParseException, SipException
    {
         ...
         * the response was meant log.trace(for this element and MUST NOT be
         ...
         * forming the best responlog.trace(se, even if this response is not chosen.
         */
        ...
        if (viaList != null && viaList.hasNext())
        {
            ...
            if ( addrMatchesInterface(viaHost) && viaPort == lps[0].getPort() )
            {
				...
				try{
                    	...
                    	SIPResponse sr = (SIPResponse) response;
						System.out.println("-----sr OK-----");
                        if(sr.getRawContent() != null) {
                            String sdpContent = new String(sr.getRawContent());
                            System.out.println("-----sdpContent OK-----");
                            SdpFactory sdpf = SdpFactory.getInstance();
                            System.out.println("-----sdpf OK-----");
                            SessionDescription requestSDP;
                            if( (requestSDP = sdpf.createSessionDescription(sdpContent)) != null )
                            {
                                log.trace("######################Notre affichage s'affiche######################");
	...
	private void sendToBB(String invite, String ok){
